...
public class SecurityCofig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(AuthenticationManagerBuilder auth) throws Exception {
		 ...
		 * 
		 * //Ajoutez le format de stockage du mot de passe, pour le texte brut,
		 * ajoutez {noop}
		 * auth.inMemoryAuthentication().withUser("admin").password("{noop}1234").roles("ADMIN","USER");
		 * auth.inMemoryAuthentication().withUser("user").password("{noop}1234").roles("USER");
		 * 
		 */
		// pour spring securie la colonne devrait normalement s'apeler pricipal
		// et pareil pour le mot de passe
		// lors de l'authentification il vérifie si la raison social existe et
		// determiner le role de l'utilisateur.
		// préciser l'encodage du mot de passe qu'il est codé en MD5
		auth.jdbcAuthentication().dataSource(dataSource)
				.usersByUsernameQuery(
						"select raison_sociale as principal,mot_de_pass as credentials,active from utilisateur where raison_sociale=?")
				.authoritiesByUsernameQuery(
						"select user_name as principal,profil_name as role from utilisateur_profil where  user_name=?")
				.rolePrefix("ROLE_");
	}
	...
	protected void configure(HttpSecurity http) throws Exception {
		// on spécifie qu'on veut une authentification basé sur un formulaire
		// avec une structure par default
		// on peut rajouter .loginPage pour customiser sa propos page de
		// connexion
		...
		// jusqu'a la prochaine connexion
		http.logout().logoutRequestMatcher(new AntPathRequestMatcher("/logout")).and().csrf().disable();

	}

	/**
	 * supprimer la sécurité au niveau des access de pages
	 */
	@Override
